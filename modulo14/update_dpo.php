...
<div class="titulo">UPDATE - DPO</div>
  ...
  if (count($_POST) > 0){
    ...
    if (!count($erros)){
      $params = [
        ...
        'site' => $dados['site'],
        'filhos' => $dados['filhos'],
        'salario' => $dados['salario'] ? str_replace(",",".",$dados['salario']) : null ,
      ];
      $result = atualizarDados($params);
      if($result !== null) {
        unset($dados);
        $sucesso['insert'] = $result ;
        header('location: exercicio.php?dir=modulo14&file=crud_dpo');
      }else{
        $erro['insert'] = $erro['update'];
      }
    }

  }
?>
<?php if(isset($erro['update'])): ?>
...
<input type="hidden" name="id" id="id" value="<?= (isset($dados['id'])? $dados['id'] : '') ?>">
  ...
  <a href="exercicio.php?dir=modulo14&file=crud_dpo" class="btn btn-success btn-lg">Voltar</a> 
</form>
